...
public class Timer : MonoBehaviour
{
	...
	private const int ClassicBaseScore = 100;
	...
	IEnumerator IEScoreBarProcess()
	{
		while (ScoreStack > 0 && GameController.action.GameState == (int)GameState.PLAYING)
		{
			...
			ScoreStack -= 1;
			if (PLayerInfo.Info.Score + 1 < 100 * PLayerInfo.MapPlayer.Level)
			//if (PLayerInfo.Info.Score + 1 < 100 )
			{ PLayerInfo.Info.Score += 1; }
			else
			{
				//PLayerInfo.Info.Score = 5000 * PLayerInfo.MapPlayer.Level;
				PLayerInfo.Info.Score = 100 * PLayerInfo.MapPlayer.Level;
				break;
			}
			float fillamount = PLayerInfo.Info.Score / (100f * PLayerInfo.MapPlayer.Level);
			Timebar.fillAmount = fillamount;
			yield return null;
		}
