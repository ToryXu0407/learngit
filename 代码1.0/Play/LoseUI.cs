...
public class LoseUI : MonoBehaviour
{
    ...
    void Start()
    {
		if (PLayerInfo.MODE != 1) {
			//playerScore = PLayerInfo.Info.Score + (PLayerInfo.MapPlayer.Level - 1) * 100;
			playerScore = PLayerInfo.Info.Score;
			for (int i = 1; i <= PLayerInfo.MapPlayer.Level; i++) {
				playerScore = playerScore + (PLayerInfo.MapPlayer.Level - 1) * i * 100;
